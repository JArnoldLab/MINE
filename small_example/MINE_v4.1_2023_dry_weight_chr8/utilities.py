...
def read_config_file(config_file):
    ...
    if os.path.exists(dict_config_file["NAME_SIMULATION"]) == False:
        os.mkdir(dict_config_file["NAME_SIMULATION"])
...
def get_initial_sigma_square(variance_info):
    variance_list = []
    for index , line in enumerate(open(variance_info)):
        for element in line.split("|")[0].strip().split(" "):
            variance = line.split("|")[2].strip()
            if float(variance) == 0:
                variance_block = 0.000000001
            else:
                variance_block = float(variance)
            variance_list.append(variance_block)
